...
```
SMS  →  МАНАЙ богино линк (bil.mn/p/<token>, урт хугацаанд хүчинтэй)
...
### ДДТД — борлуулагчийн дугаар vs багцын дугаар (B-70)

POS API 3.0-ийн `POST /rest/receipt` хариу ХОЁР түвшний ДДТД агуулна:

| Талбар | Юу вэ | Угтвар |
|---|---|---|
| `id` (top-level) | Багц баримтын ДДТД. Операторын POS-оор дамжуулсан (LIME) загварт багцын `merchantTin` = ОПЕРАТОРЫН ТТД | операторын ТТД (ж: `02910024410…`) |
| `receipts[].id` | Борлуулагчийн дэд баримтын ДДТД (`merchantTin` = tenant-ийн ТТД). **ebarimt.mn дээр tenant-ийн нэр дээр бүртгэгддэг дугаар нь ЭНЭ** | tenant-ийн ТТД (ж: `01520002009…`) |

2026-09-с өмнө adapter зөвхөн top-level `id`-г `receiptNo` болгож хадгалдаг
байсан тул хэрэглэгчид өгсөн дугаар порталтай хэзээ ч таардаггүй байв. Одоо:

- `receiptNo` = борлуулагчийн `receipts[].id` (merchantTin = tenant ТТД-тэй мөр;
  олдохгүй бол ганц дэд баримт; тэр ч байхгүй бол top-level `id` + warn лог).
  API (`receipt_no`), webhook (`receipt_no`), dashboard бүгд үүнийг харуулна.
- `batchReceiptNo` = top-level `id`. `DELETE /rest/receipt {id, date}` үүгээр
  хийгдэнэ (өмнөх мөрүүдэд `batchReceiptNo` NULL → `receiptNo` өөрөө багцынх).
- `providerResponse` = POS API-ийн түүхий хариу, `lottery`/`qrData`-г ХАССАН
  (ТЕГ-ийн хадгалалтын хориг) — тулгалт/маргаанд нотолгоо.

### Нэг хүсэлт → ТЕГ дээр хоёр баримт үүсэхээс хамгаалалт (B-70)

- `processOne` provider руу илгээхийн ӨМНӨ мөрийг `lockedAt`-аар claim хийнэ
  (3 мин TTL). Өмнө нь claim ТЕГ-ийн хариуны ДАРАА хийгддэг тул
  `createStandalone`-ийн синхрон дуудлага ба 10 минутын sweeper / төлбөрийн
  дараах `processPending` нэг PENDING мөрийг зэрэг аваад ТЕГ дээр хоёр баримт
  үүсгэж, хоёр дахь нь хаана ч бүртгэгдэлгүй орхигдох боломжтой байв.
- Partner API-ийн `Idempotency-Key` нь хүсэлтийг ажиллуулахаас ӨМНӨ
  бүртгэгдэнэ; боловсруулагдаж байхад давтвал `409 IDEMPOTENCY_IN_PROGRESS`.
  Өмнө нь хариу ирсний дараа л бичигддэг тул хэрэглэгчийн timeout-retry ТЕГ рүү
  давхар очдог байв.
